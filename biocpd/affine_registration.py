...
class AffineRegistration(EMRegistration):
    ...
    def __init__(self, B=None, t=None, use_kdtree=True, k=10, *args, **kwargs):
    ...
    def update_transform(self):
        ...
        # YPY = Y_hat^T diag(P1) Y_hat  -> use broadcasting
        YPY = (self.P1[:, None] * Y_hat).T @ Y_hat
        self.YPY = YPY

        # Solve for B with Cholesky (add small jitter for stability)
        jitter = 1e-8
        c, low = cho_factor(self.YPY + jitter * np.eye(self.D))
        self.B = cho_solve((c, low), self.A.T).T
        self.t = (muX - (self.B.T @ muY.T).T)
